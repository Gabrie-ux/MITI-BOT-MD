...
global.authFile = `Kenny-BotSession`
...
browser: opcion == '1' ? ['Kenny_Bot', 'Edge', '20.0.04'] : methodCodeQR ? ['Kenny-Bot', 'Edge', '20.0.04'] : ["Ubuntu", "Edge", "20.0.04"],
auth: {
creds: state.creds,
...
let prekey = []
let directorio = readdirSync("./Kenny-SubBots")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') /*|| file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-') */
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./Kenny-BotSession/${files}`)
})
} 
...
if (statSync(`./Kenny-SubBots/${directorio}`).isDirectory()) {
let DSBPreKeys = readdirSync(`./Kenny-SubBots/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-') /*|| fileInDir.startsWith('app-') || fileInDir.startsWith('session-')*/
})
SBprekey = [...SBprekey, ...DSBPreKeys]
DSBPreKeys.forEach(fileInDir => {
unlinkSync(`./Kenny-SubBots/${directorio}/${fileInDir}`)
})
}
})
...
const directories = ['./Kenny-BotSession/', './Alya-SubBots/']
...
conn.bye = 'Que triste que se fue @user \n\nQue dios lo bendiga `'
conn.spromote = ' @user ya forma parte de staff 👑'
